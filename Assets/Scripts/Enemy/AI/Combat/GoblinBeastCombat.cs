...
namespace Enemy.AI.Combat
{
    ...
    [RequireComponent(typeof(GoblinBeastParticle))]
    public class GoblinBeastCombat : BossCombat
    {
        ...
        public float Attack1Scale => beastAttackSlam.radius;
        public float Attack2Scale => beastAttackBite.radius;

        [HideInInspector] public UnityEvent onSlam, onBite;
        ...
        protected virtual IEnumerator SphereAttack(BeastAttackBite meleeBite)
        {
            ...
            foreach (var newTime in meleeBite.applyDamageOn.Select(time => time - meleeBite.UsedTime))
            {
                ...
                var tra = transform;
                AttackSphereBite(meleeBite.localAttackPosition + tra.position + tra.forward, meleeBite.radius, meleeBite.damage, meleeBite.heal, meleeBite.targetLayer);
            }
        ...
        protected virtual IEnumerator SphereAttack(BeastAttackStagger meleeStagger)
        {
            ...
            foreach (var newTime in meleeStagger.applyDamageOn.Select(time => time - meleeStagger.UsedTime))
            {
                ...
                onSlam?.Invoke();
                StaggerAttackSphere(transform.position + meleeStagger.localAttackPosition, meleeStagger.radius, meleeStagger.damage, meleeStagger.time, meleeStagger.targetLayer);
            }
            rotateToTarget = false;
        }
        ...
        {
            onBite?.Invoke();
            var foundTargets = CombatCollider.GetTargets(position, radius, targetLayer);

            foreach (var iTarget in foundTargets)
            {
                iTarget.ForceReceiveDamage(damage);
        ...
        protected virtual void StaggerAttackSphere(Vector3 position, float radius, float damage, float time, LayerMask targetLayer)
        {
            ...
            foreach (var iTarget in foundTargets)
            {
                iTarget.ForceDisableMovement(time);
                iTarget.ForceReceiveDamage(damage);
            }
        }
